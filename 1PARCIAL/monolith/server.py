...
class BlogHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        data=HTTPDataHandler.handle_response(self)
        ...
        if self.path == "/posts":
            ...
            data2 = HTTPDataHandler.handle_reader({"id": new_post_id})
        else:
            self.send_error(404, "Ruta no válida")
    ...
    def do_PUT(self):
        ...
        if self.path.startswith("/post/"):
            ...
            if post_id in db:
                ...
                db[post_id]["title"] = post_params.get("title", [db[post_id]["title"]])[0]
                db[post_id]["content"] = post_params.get("content", [db[post_id]["content"]])[0]
